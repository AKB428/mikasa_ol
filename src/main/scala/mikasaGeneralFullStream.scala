 ...
 */
object MikasaGeneralFullStream {
...
//      val tokenizer : Tokenizer = CustomTwitterTokenizer.builder().build()  // kuromojiの分析器
      ...
      if(japanese_pattern.matcher(tweetText).find()) {  // ひらがなが含まれているツイートのみ処理
        ...
        // ツイート本文の解析
        val tokens : java.util.List[Token] = CustomTwitterTokenizerF.tokenize(tweetText, dictFilePath)
    ...
    ).reduceByKeyAndWindow(_+_, Seconds(60 * 60 * 24)   // ウインドウ幅(60*60sec)に含まれる単語を集める
...
object CustomTwitterTokenizerF {
